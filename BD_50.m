% This program is to study the Block-Diagonal (BD) model with s=50, using
% the synthetic data.
...
load A_Matrix_big_BD
load RandomVector_big_BD
nmax = 25000;
...
for nshuffle = 1:nshuffle_max
 ...
 genotype = A_Matrix_big_BD(:,randperm(p));
...
random = zeros(n,1);
random(1:n) = RandomVector_big_BD(1:n); 
y3 = sqrt(1-h2)*random;
...
dlmwrite('BD_50_xhatfirst0',xhat_first_collect0,'delimiter',',')
dlmwrite('BD_50_xhatfirst',xhat_first_collect,'delimiter',',')
dlmwrite('BD_50_0var',ResidualVar,'delimiter',',')
dlmwrite('BD_50_nstar',nstar,'delimiter',',')
dlmwrite('BD_50_var',GXvar,'delimiter',',')
dlmwrite('BD_50_L2',GXL2,'delimiter',',')
dlmwrite('BD_50_false',false,'delimiter',',')
